...
def get_max_min(df, smoothing, window_range, neighbour, prominence, debug=False):
    try:
        ...
        message = "Exception in get_max_min: {} {}".format(df['sid'].iloc[-1], e)
        print(message)
        return ''
...
def show_patterns(sid, patterns):
    if len(patterns) > 0:
        print(sid)
        for k, v in patterns.items():
            for pattern in v:
                print('pattern range from {} to {}'.format(pattern[0], pattern[1]))

def detect_single(sid, df):
    minmax = get_max_min(df, smoothing, window, neighbour, prominence, debug=True)
